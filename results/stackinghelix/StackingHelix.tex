...
alphaA = Pi/3;
alphaB = -Pi/3;
betaA = (Pi/2 -  ArcTan[2 Sqrt[2]] Cos[Pi/ 6]);
betaB = (Pi/2 - ArcTan[2 Sqrt[2]] Cos[Pi/ 6]);

omegaFromAB[aa_,ab_,ba_,bb_,tau_] :=
ab + aa Cos[tau] + ba Sin[tau]

rhoFromAB[aa_,ab_,ba_,bb_,tau_] :=
bb + ba Cos[tau] + aa Sin[tau]

N[omegaFromAB[alphaA,alphaB,betaA,betaB,2 Pi/3] 180 / Pi]
N[rhoFromAB[alphaA,alphaB,betaA,betaB,2 Pi/3] 180 / Pi]

(* Now a serious test for correctness is if we use these rho and omega (for a tetrahedron),
does the delix correspond to the BC tetrahelix? And what does ``correspond mean''?)


(* A sanity check is that if tau = 0, omega should be 0 and rho five times should almost be 2Pi *)
alphaR = 0;
betaR = ArcTan[2 Sqrt[2]]/2;
omegaFromAB[alphaR,alphaR,betaR,betaR,0]
(* This should be about 10 degrees *)
N[(2Pi - rhoFromAB[alphaR,alphaR,betaR,betaR,0] 5 ) 180 / Pi]

N[UnifiedComp5[1/3,rhoFromAB[alphaA,alphaB,betaA,betaB,2 Pi/3],
    omegaFromAB[alphaA,alphaB,betaA,betaB,2 Pi/3] 180 / Pi]]

N[UnifiedComp5[1/3,33.15 Pi / 180,
    -6.587 Pi / 180]
